          ...
          <div className="flex flex-col items-center w-full pb-20">
            ...
            <div className="w-full flex-col flex items-center mb-12">
               <div className="grid grid-cols-1 md:grid-cols-4 w-full md:h-[300px] mb-8">
                 {displayProducts.filter(p => p.nombre.toLowerCase().includes('sombrero')).map((p, i) => (
                    <div key={p.id} className={`flex flex-col justify-center items-center relative py-12 md:py-0 ${i % 2 === 1 ? 'bg-black text-white' : 'bg-white text-black'}`}>
                      {p.imagen ? (
                        <img src={p.imagen} className="w-48 h-48 object-cover z-10 hover:scale-110 transition-transform" alt={p.nombre} />
                      ) : (
                        <div className="w-48 h-48 bg-gray-200"></div>
                      )}
                      <div className="text-gus-gold mt-4 font-serif text-lg">{p.precio / 100} €</div>
            ...
            <div className="w-full relative py-20 flex flex-col md:flex-row px-8 items-center justify-between gap-12 md:gap-0">
              <div className="flex flex-col md:flex-row gap-8 items-end relative z-10 md:w-1/3">
                 {displayProducts.filter(p => p.nombre.toLowerCase().includes('bolso') || p.nombre.toLowerCase().includes('clutch')).slice(0, 2).map((p, i) => (
                    <div key={p.id} className="flex flex-col items-center relative">
                      ...
                      {p.imagen ? (
                        <img src={p.imagen} className="w-40 h-40 object-cover hover:scale-110 transition-transform z-10" alt={p.nombre} />
                      ) : (
                        <div className="w-40 h-40 bg-gray-200"></div>
                      )}
                      <div className="text-gus-gold mt-4 font-serif font-bold text-sm bg-white px-2 rounded z-10 shadow-sm">{p.precio / 100} € <span className="bg-[#222] text-white px-3 py-1 rounded-full font-serif italic text-xs ml-2 cursor-pointer hover:bg-gus-gold transition-colors">Comprar</span></div>
                    </div>
                 ))}
              </div>
              
              <div className="md:w-1/3 text-center px-4 md:px-8 z-10">
                 <h3 className="font-logo text-4xl leading-tight bg-white md:bg-transparent py-4 text-black">Lleva contigo algo más que estilo: lleva confianza.</h3>
              </div>
              ...
              <div className="md:w-1/3 relative flex flex-col md:flex-row justify-center items-center gap-8 md:gap-4 z-10 pr-0 md:pr-8">
                 ...
                 {displayProducts.filter(p => p.nombre.toLowerCase().includes('bolso') || p.nombre.toLowerCase().includes('clutch')).slice(2, 4).map(p => (
                   <div key={p.id} className="flex flex-col items-center">
                     <div className="hidden md:block w-1 h-8 bg-gray-600 rounded"></div>
                     {p.imagen ? (
                        <img src={p.imagen} className="w-32 h-32 object-cover shadow-lg bg-white rounded" alt={p.nombre} />
                      ) : (
