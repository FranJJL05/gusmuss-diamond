...
class ProductController extends AbstractController
{
    ...
    public function list(Request $request): JsonResponse
    {
        ...
        return $this->json(
            array_map(fn($p) => $this->serializeProduct($p), $products),
            200,
            $this->corsHeaders()
        );
    }
    ...
    public function show(string $slug): JsonResponse
    {
        ...
        return $this->json($this->serializeProduct($product, true), 200, $this->corsHeaders());
    }

    /**
     * Convierte un Product a array serializable para la respuesta JSON.
     */
    private function serializeProduct(\App\Entity\Product $p, bool $full = false): array
    {
        $data = [
            'id'              => $p->getId(),
            'nombre'          => $p->getNombre(),
